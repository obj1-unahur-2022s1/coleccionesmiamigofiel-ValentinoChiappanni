...
object woody {
	...
	method cantCaracteresFrases() = frasesDelCordon.sum( { frase => frase.size() } )
...
object rex {
	...
	method buenEstado() = self.companiero().buenEstado()
	method nombreOriginal() = "Rex"
	method restaurarValorInicial() { potenciador = 1  }
}
